...
datafitppRefExtrapolation(){
  ...
  TFile *file5 = new TFile("R3_ScaleFactor2760GeVto5TeVTGraphErrors.root","read");
...
*/
    int msty[nEta+2]={25,27,28,30,30,24};
    int mcol[nEta+2]={1,4,2,2,2,4};
    float msiz[nEta+2]={1.2,1.4,1.3,1.4,1.4,1.2};
  for(int i=0; i<nEta; i++){
   nBin=graph[i]->GetN();
...
*/
  ...
  for(int i=0; i<nEta; i++){
  if(i==0||i==4||i==5)graph2[i]->Draw("psameEZ");  
...
if(i<6){
    ...
    leg0->AddEntry(graph2[4],"2.76 TeV, R=0.3 + Z2","p");
    leg0->AddEntry(graph2[5],"7 TeV, R=0.5 + DATA","p");
    leg0->Draw();

c1->Print("ppCrossSectionExtrapolationdata.gif");
 ...
 hist2->Draw();
  ...
  if(i==4)gr2[i] = new TGraphErrors(22, ptx[i], fratio[i], 0, ratioError[i]);
  if(i==5)gr2[i] = new TGraphErrors(22, ptx[i], fratio[i], 0, ratioError[i]);
  ...
  if(i==0||i==4||i==5)gr2[i]->Draw("psameEZ");
  }
    ...
    leg0->AddEntry(gr2[4],"(2.76 TeV, R=0.3 + Z2)/(Fitted Default)","p");
...
c2->Print("ppFitExtrapolationRatiodata.gif");

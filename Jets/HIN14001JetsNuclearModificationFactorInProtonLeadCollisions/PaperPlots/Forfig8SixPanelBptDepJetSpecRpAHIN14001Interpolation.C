...
void Forfig8SixPanelBptDepJetSpecRpAHIN14001Interpolation(Display_t dis = kRpA, Level_t lev = kUnfold, DataSet_t data=kAll)
{
    TH1::SetDefaultSumw2();
...
//xxx
 ...
 //               t2->AddEntry(PARatio[ieta], Form("%.1f < #eta_{CM} < %.1f", deta[netabin-ieta-1],deta[netabin-ieta]),"P");
   ...
   lines->SetLineWidth(1);
      ...
      for(int i=1; i<20; i++){
         sys[ieta][i]=sqrt(ppsys[ieta][i]*ppsys[ieta][i]+sys[ieta][i]*sys[ieta][i]);
      }
            ...
            for(int ieta = 0 ; ieta <=netabin-2; ieta+=1){
              ...
              drawSys(PARatio[ieta], ptboundary[ieta], ptCen[ieta], sys[ieta], 5, 0, ieta);
...
//            }
    ...
    if(SavePlot)c1->Print(Form("%s/%s%sIncJet%s%s%s%sInterpolatedRefEtaBin%d6panelF.gif", plotsdir.Data(), val.Data(), algo.Data(),residual.Data(), ana.Data(), widebin.Data(), dep.Data(),netabin));
    if(SavePlot)c1->Print(Form("%s/%s%sIncJet%s%s%s%sInterpolatedRefEtaBin%d6panelF.C", plotsdir.Data(), val.Data(), algo.Data(),residual.Data(), ana.Data(), widebin.Data(), dep.Data(),netabin));
    if(SavePlot)c1->Print(Form("%s/%s%sIncJet%s%s%s%sInterpolatedRefEtaBin%d6panelF.pdf", plotsdir.Data(), val.Data(), algo.Data(),residual.Data(), ana.Data(), widebin.Data(), dep.Data(),netabin));
    c1->Update();

...
void Forfig8ThreePanelBptDepJetSpecRpAHIN14001Interpolation(Display_t dis = kRpA, Level_t lev = kUnfold, DataSet_t data=kAll)
{
    TH1::SetDefaultSumw2();
...
//xxx
            ...
            for(int ieta = 0 ; ieta <=netabin-2; ieta+=1){
                  if(ieta==1||ieta==2||ieta==4) continue;
                  if(ieta==0) c1->cd(1);
                  if(ieta==3) c1->cd(2);
                  if(ieta==5) c1->cd(3);
...
//            }
    ...
    if(SavePlot)c1->Print(Form("%s/%s%sIncJet%s%s%s%sInterpolatedRefEtaBin%d3panelB.gif", plotsdir.Data(), val.Data(), algo.Data(),residual.Data(), ana.Data(), widebin.Data(), dep.Data(),netabin));
    if(SavePlot)c1->Print(Form("%s/%s%sIncJet%s%s%s%sInterpolatedRefEtaBin%d3panelB.C", plotsdir.Data(), val.Data(), algo.Data(),residual.Data(), ana.Data(), widebin.Data(), dep.Data(),netabin));
    c1->Update();

...
void Forfig8SixPanelCptDepJetSpecRpAHIN14001Interpolation(Display_t dis = kRpA, Level_t lev = kUnfold, DataSet_t data=kAll)
{
    TH1::SetDefaultSumw2();
    ...
    TString plotsdir = "/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/plots";
        //const char * kHomeDir = "/Users/ymao/group/CMS/anaOutputs/pPb/RpPb" ;
        const char * kHomeDir = "/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log" ;
...
//        {286101, -47.4919, -5.34629, 6.8595, -1.05411}};
    ...
    for(int ieta = 0 ; ieta <=netabin; ieta++){
        if(ieta==netabin){
            if(dis==kSpecPb || dis==kRpA || dis==kRpAEtaDep)
                ...
                inname = Form("/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/sys/sysSpectrumTotalEtaBin%.f_%.f.txt", -1.0*10, 1.0*10);
            else
               //inname =Form("/Users/ymao/group/CMS/Sys/HIN-14-001/sysAsymmetetryTotalEtaBin%.f_%.f.txt",-1.0*10, 1.0*10);
               inname =Form("/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/sys/sysAsymmetetryTotalEtaBin%.f_%.f.txt",-1.0*10, 1.0*10);
        }
        else {
            if(dis==kSpecPb || dis==kRpA || dis==kRpAEtaDep)
                //inname = Form("/Users/ymao/group/CMS/Sys/HIN-14-001/sysSpectrumTotalEtaBin%.f_%.f.txt",deta[ieta]*10, deta[ieta+1]*10);
                inname = Form("/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/sys/sysSpectrumTotalEtaBin%.f_%.f.txt",deta[ieta]*10, deta[ieta+1]*10);
            else
                //inname =Form("/Users/ymao/group/CMS/Sys/HIN-14-001/sysAsymmetetryTotalEtaBin%.f_%.f.txt",deta[ieta]*10, deta[ieta+1]*10);
                inname =Form("/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/sys/sysAsymmetetryTotalEtaBin%.f_%.f.txt",deta[ieta]*10, deta[ieta+1]*10);
        }
    ...
    for(int ieta = 0 ; ieta <=netabin; ieta++){
        if(ieta==netabin){
            ...
            //innamepp = Form("/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/pp/ppReferenceFrom7TeVR5SysEtaBin%.f_%.f.txt", -1.0*10, 1.0*10);
            innamepp = Form("/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/pp/ppRefSysTotalEtaBin%.f_%.f.txt", -1.0*10, 1.0*10);
            //            else
            ...
            //innamepp = Form("/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/pp/ppReferenceFrom7TeVR5SysEtaBin%.f_%.f.txt",deta[ieta]*10, deta[ieta+1]*10);
            innamepp = Form("/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/pp/ppRefSysTotalEtaBin%.f_%.f.txt",deta[ieta]*10, deta[ieta+1]*10);
            //            else
    ...
    fcorr = TFile::Open("/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/QCD10011Binning/correctionfactor.root", "readonly");
    ...
    if(UsePYTHIARef){
        ...
        reffile = new TFile(Form("/Users/tuos/Analysis/CMS/eps09/jetpp/gplot9May17log/pp/R3_ScaleFactor7TeVto5TeVStartingCone5Sys-2.root"));
   ...
   lines->SetLineWidth(1);
      ...
      for(int i=1; i<20; i++){
       //if(ieta==3) cout<<"test ptcen :"<<ptCen[ieta][i]<<"  "<<PARatio[ieta]->GetBinContent(i+14)<<"  "<<ppsys[ieta][i]<<"  "<<sys[ieta][i]<<endl;
        lines->DrawLine(ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+ppsys[ieta][i]*0.01),
                        ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+ppsys[ieta][i]*0.01));
        lines->DrawLine(ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+ppsys[ieta][i]*0.01)-0.02,
                        ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+ppsys[ieta][i]*0.01));
        lines->DrawLine(ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+ppsys[ieta][i]*0.01)-0.02,
                        ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+ppsys[ieta][i]*0.01));
        lines->DrawLine(ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-ppsys[ieta][i]*0.01),
                        ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-ppsys[ieta][i]*0.01));
        lines->DrawLine(ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-ppsys[ieta][i]*0.01),
                        ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-ppsys[ieta][i]*0.01)+0.02);
        lines->DrawLine(ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-ppsys[ieta][i]*0.01),
                        ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-ppsys[ieta][i]*0.01)+0.02);

        lines->DrawLine(ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+sys[ieta][i]*0.01),
                        ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+sys[ieta][i]*0.01));
        lines->DrawLine(ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+sys[ieta][i]*0.01)-0.02,
                        ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+sys[ieta][i]*0.01));
        lines->DrawLine(ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+sys[ieta][i]*0.01)-0.02,
                        ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1+sys[ieta][i]*0.01));
        lines->DrawLine(ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-sys[ieta][i]*0.01),
                        ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-sys[ieta][i]*0.01));
        lines->DrawLine(ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-sys[ieta][i]*0.01),
                        ptCen[ieta][i]-ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-sys[ieta][i]*0.01)+0.02);
        lines->DrawLine(ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-sys[ieta][i]*0.01),
                        ptCen[ieta][i]+ptCen[ieta][i]*0.03,  PARatio[ieta]->GetBinContent(i+14)*(1-sys[ieta][i]*0.01)+0.02);
            ...
            TBox *box = new TBox(670-670*0.09,1.0-0.035,670+670*0.03,1.0+0.035);
...
*/            
            ...
            c1->cd(1);
            TBox *box1 = new TBox(95.,1.70,125.5,1.76);
            box1->SetFillColor(5);
            box1->SetLineColor(5);
            //box1->SetFillStyle(1001);
            box1->Draw("same");
            TLatex *tex1= new TLatex(145.0,1.70,"pp uncertainties");
            tex1->SetTextColor(1);
            tex1->SetTextSize(20);
            tex1->SetTextFont(43);
            tex1->Draw();
            TBox *box2 = new TBox(95.,1.54,125.5,1.6);
         //   box2->SetFillColor(kYellow);
            ...
            //box2->SetLineWidth(2);
            box2->Draw("same");
            TLatex *tex1= new TLatex(145.0,1.54,"pPb uncertainties");
            tex1->SetTextColor(1);
            tex1->SetTextSize(20);
            tex1->SetTextFont(43);
            tex1->Draw();
            ...
            //t2->Draw("same");
            c1->cd(3);
            TBox *box1 = new TBox(62.8,1.63,79.5,1.69);
            box1->SetFillColor(kGreen+3);
            box1->SetLineColor(kGreen+3);
            box1->SetFillStyle(1001);
            box1->Draw("same");
            TLatex *tex1= new TLatex(88.0,1.63,"Luminosity uncertainties");
            tex1->SetTextColor(1);
            tex1->SetTextSize(20);
            tex1->SetTextFont(43);
            tex1->Draw();
...
//            }
    ...
    if(SavePlot)c1->Print(Form("%s/%s%sIncJet%s%s%s%sInterpolatedRefEtaBin%d6panel.gif", plotsdir.Data(), val.Data(), algo.Data(),residual.Data(), ana.Data(), widebin.Data(), dep.Data(),netabin));
    if(SavePlot)c1->Print(Form("%s/%s%sIncJet%s%s%s%sInterpolatedRefEtaBin%d6panel.C", plotsdir.Data(), val.Data(), algo.Data(),residual.Data(), ana.Data(), widebin.Data(), dep.Data(),netabin));
    if(SavePlot)c1->Print(Form("%s/%s%sIncJet%s%s%s%sInterpolatedRefEtaBin%d6panel.pdf", plotsdir.Data(), val.Data(), algo.Data(),residual.Data(), ana.Data(), widebin.Data(), dep.Data(),netabin));
    c1->Update();

...
void fig9JetSpecRpAHIN14001Interpolation(Display_t dis = kRpAEtaDep, Level_t lev = kUnfold, DataSet_t data=kAll)
//void JetSpecRpAHIN14001Interpolation(Display_t dis = kRpA, Level_t lev = kUnfold, DataSet_t data=kAll)
{
    TH1::SetDefaultSumw2();
...
//        FBRatio[ieta] = (TH1F*)rebinjet[netabin-ieta-1]->Clone(Form("JetAsymmetry_Eta%.f_%.f", deta[netabin-ieta-1]*10, deta[netabin-ieta]*10));
    ...
    if(SaveFile){
        TFile * outf = new TFile(Form("%s/%sJetTrigPYTHIA%sJet%sSpectraRpA%sForEricMay8.root", kHomeDir, coll.Data(), algo.Data(), residual.Data(),class.Data()), "RECREATE");
...
//    c1->SetFrameBorderMode(0);
    ...
    TCanvas *c1 = new TCanvas("c1", "c1",99,26,599,599);
    gStyle->SetOptFit(1);
    gStyle->SetOptStat(0);
    gStyle->SetOptTitle(0);
...
//    c1->SetLogy();
    c1->SetTickx(1);
    c1->SetTicky(1);
    c1->SetLeftMargin(0.1352174);
    c1->SetRightMargin(0.02434783);
    c1->SetTopMargin(0.08166969);
    c1->SetBottomMargin(0.1388566);
    c1->SetFrameFillStyle(0);
...
//    hDum->GetXaxis()->SetRangeUser(20., 600.);
    hDum->GetXaxis()->SetTitleSize(0.056);
    hDum->GetXaxis()->SetTitleOffset(1.15);
    hDum->GetXaxis()->SetLabelSize(0.05);
    hDum->GetYaxis()->SetTitleSize(0.05);
    hDum->GetYaxis()->SetLabelSize(0.045);
    hDum->GetYaxis()->SetTitleOffset(1.25);
...
//            hhdum->GetYaxis()->SetTitle("1/N_{ev}d^{2}N/dp_{T}d#eta");
        ...
        case kRpA:
            ...
            hDum->SetMinimum(0.66);
            hDum->SetMaximum(2.1);
            }
            if(netabin==1)
                hDum->SetAxisRange(47.5, 700., "X");
            else
                hDum->SetAxisRange(47.5, 990., "X");
            if(IsMC)
                hDum->GetYaxis()->SetTitle("Reco/Truth");
            else
                hDum->GetYaxis()->SetTitle("R_{pPb}");
//                hDum->GetYaxis()->SetTitle("R_{pPb}^{PYTHIA}");
            hDum->Draw();
            ...
            {52.5, 60, 69, 79, 90.5, 105.5, 123.5, 143, 163.5, 185, 208, 232.5, 258.5, 286, 315, 346, 378.5, 412.5, 449}};
            for(int s=0; s<19; s++){
              veta3[0][s]=veta3[0][s]*0.98;
              veta3[2][s]=veta3[2][s]*1.02;
            }
...
//                RpPbsys2[2][i]=sys[i][10];
 ...
 //               t2->AddEntry(PARatio[ieta], Form("%.1f < #eta_{CM} < %.1f", deta[netabin-ieta-1],deta[netabin-ieta]),"P");
            ...
            ///for(int ieta = 0 ; ieta <=netabin; ieta+=3) PARatio[ieta]->DrawCopy("same");
...
//            t2->AddEntry(PARatio[6], Form("%.1f < #eta_{CM} < %.1f", deta[netabin-6-1],deta[netabin-6]),"P");
            ...
            for(int j=0; j<3; j++){
                ...
                while (np < npts[j]) {
                    ...
                    TBox *box1 = new TBox(veta3[j][np]-pct*veta3[j][np],vRpPb3[j][np]-ppsys3[j][np],veta3[j][np]+pct*veta3[j][np],vRpPb3[j][np]+ppsys3[j][np]);
            ...
            for(int j=0; j<3; j++){
                int np=0;
                while (np < npts[j]) {
                    TBox *box = new TBox(veta3[j][np]-pct*veta3[j][np],vRpPb3[j][np]-RpPbsys3[j][np],veta3[j][np]+pct*veta3[j][np],vRpPb3[j][np]+RpPbsys3[j][np]);
            ...
            TBox *box1 = new TBox(318.,1.58,335.5,1.61);
            box1->SetFillColor(7);
            box1->SetLineColor(7);
            box1->SetFillStyle(1001);
            box1->Draw("same");
            TLatex *tex1= new TLatex(365.0,1.58,"pp uncertainties");
            tex1->SetTextColor(1);
            tex1->SetTextSize(20);
            tex1->SetTextFont(43);
            tex1->Draw();
            TBox *box2 = new TBox(318.,1.5,335.5,1.53);
         //   box2->SetFillColor(kYellow);
            box2->SetFillStyle(0);
            box2->SetLineColor(kBlack);
            box2->SetLineWidth(2);
            box2->Draw("same");
            TLatex *tex1= new TLatex(365.0,1.5,"pPb uncertainties");
            tex1->SetTextColor(1);
            tex1->SetTextSize(20);
            tex1->SetTextFont(43);
            tex1->Draw();
            ...
            TBox *box = new TBox(850-850*0.03,1.0-0.035,850+850*0.03,1.0+0.035);
            box->SetFillColor(kMagenta);
            box->SetFillStyle(-1);
            box->Draw();

            TGraphErrors *graph = new TGraphErrors(13,veta3[0],vRpPb3[0],0,RpPbstat3[0]);
            graph->SetTitle("gr");
            ...
            //drawSysTGraph(graph, veta3[0], ppsys3[0], 7, 1001, 1);
            //drawSysTGraph(graph, veta3[0], RpPbsys3[0], colorCode[0], 0, 1);
            graph->Draw("psameez");
            TGraphErrors *graph2 = new TGraphErrors(18,veta3[1],vRpPb3[1],0,RpPbstat3[1]);
            graph2->SetTitle("gr2");
            ...
            //drawSysTGraph(graph2, veta3[1], ppsys3[1], 5, 1001, 1);
            //drawSysTGraph(graph2, veta3[1], RpPbsys3[1], colorCode[1], 0, 2);
            graph2->SetMarkerSize(1.5);
            graph2->Draw("psameez");
            TGraphErrors *graph3 = new TGraphErrors(13,veta3[2],vRpPb3[2],0,RpPbstat3[2]);
            graph3->SetTitle("gr3");
            ...
            //drawSysTGraph(graph3, veta3[2], ppsys3[2], 16, 1001, 1);
            //drawSysTGraph(graph3, veta3[2], RpPbsys3[2], colorCode[2], 0, 3);
            graph3->Draw("psameez");
...
//                t3->AddEntry(FBRatio[ieta], Form("%.1f<#eta_{CM}<%.1f/%.1f< #eta_{CM}<%.1f",  deta[ieta],deta[ieta+1],deta[netabin-ieta-1],deta[netabin-ieta]),"P");
        ...
        case kRpAEtaDep:
            ...
            TH1D* hist2 = new TH1D("hist2","hist2",100,-2.2,1.8);
   ...
   //         hist2->SetYTitle("R_{pPb}^{PYTHIA}");
            ...
            double errorXeta[8]={0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25};
            double errorXetal0[8]={0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15};
            double errorXetah0[8]={0.35, 0.35, 0.35, 0.35, 0.35, 0.35, 0.35, 0.35};
            double errorXetal1[8]={0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25};
            double errorXetah1[8]={0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25};
            double errorXetal2[8]={0.35, 0.35, 0.35, 0.35, 0.35, 0.35, 0.35, 0.35};
            double errorXetah2[8]={0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15};
            for(int i=0; i<8; i++){
              errorXeta[i]=0;
              errorXetal0[i]=0;
              errorXetah0[i]=0;
            ...
            drawText(Form("Anti-k_{T} Particle Flow Jets: R = 0.3"),0.2,0.855,20);
            drawText(Form("CMS Preliminary, pPb #sqrt{s_{NN}} = 5.02 TeV "),0.25,0.95,20);
...
//xxx
            ...
            TBox *box = new TBox(1.57-0.04,1.0-0.035,1.57+0.08,1.0+0.035);
            box->SetFillColor(kGreen+3);
            box->SetFillStyle(-1);
            box->Draw();
            TGraphErrors *graph = new TGraphErrors(6,veta2[0],vRpPb2[0],errorXeta,RpPbstat2[0]);
            ...
            //graph->Draw("psameez");
            TGraphAsymmErrors *graphAs = new TGraphAsymmErrors(6,veta2[0],vRpPb2[0],errorXetal0, errorXetah0,RpPbstat2[0],RpPbstat2[0]);
            graphAs->SetTitle("gr");
            graphAs->SetMarkerStyle(33);
            graphAs->SetMarkerColor(2);
            graphAs->SetLineColor(2);
            graphAs->SetLineWidth(2);
            graphAs->SetMarkerSize(1.5);
            graphAs->Draw("psameez");

            TGraphErrors *graph2 = new TGraphErrors(6,veta2[1],vRpPb2[1],errorXeta,RpPbstat2[1]);
            graph2->SetTitle("gr2");
            graph2->SetMarkerStyle(20);
            ...
            drawSysTGraph(graph2, veta2[1], ppsys2[1], 7, 1001, 1);
            drawSysTGraph(graph2, veta2[1], RpPbsys2[1], colorCode[1], 0, 2);
            graph2->SetMarkerSize(1.5);
            graph2->Draw("psameez");
            TGraphErrors *graph3 = new TGraphErrors(6,veta2[2],vRpPb2[2],errorXeta,RpPbstat2[2]);
            ...
            //graph3->Draw("psameez");
            TGraphAsymmErrors *graph3As = new TGraphAsymmErrors(6,veta2[2],vRpPb2[2],errorXetal2,errorXetah2,RpPbstat2[2],RpPbstat2[2]);
            graph3As->SetTitle("gr3");
            graph3As->SetMarkerStyle(27);
            graph3As->SetMarkerColor(4);
            graph3As->SetLineColor(4);
            graph3As->SetLineWidth(2);
            graph3As->SetMarkerSize(1.5);
            graph3As->Draw("psameez");            

            regSun(-2.2,1.,1.7,1.,1, 1);

            
            TLegend *leg0 = new TLegend(0.54,0.665,0.73,0.825);
...
*/
            leg0->Draw();
            TBox *box1 = new TBox(-0.235,1.55,-0.07,1.59);
            box1->SetFillColor(7);
            box1->SetLineColor(7);
            box1->SetFillStyle(1001);
            box1->Draw("same");
            TLatex *tex1= new TLatex(0.02,1.555,"pp uncertainties");
            tex1->SetTextColor(1);
            tex1->SetTextSize(20);
            tex1->SetTextFont(43);
            tex1->Draw();
            TBox *box2 = new TBox(-0.23,1.46,-0.075,1.495);
         //   box2->SetFillColor(kYellow);
            box2->SetFillStyle(0);
            box2->SetLineColor(kBlack);
            box2->SetLineWidth(2);
            box2->Draw("same");
            TLatex *tex1= new TLatex(0.02,1.46,"pPb uncertainties");
            tex1->SetTextColor(1);
            tex1->SetTextSize(20);
            tex1->SetTextFont(43);
            tex1->Draw();

            TBox *box1 = new TBox(-0.5,0.75,-0.33,0.79);
            box1->SetFillColor(kGreen+3);
            box1->SetLineColor(kGreen+3);
            box1->SetFillStyle(1001);
            box1->Draw("same");
            TLatex *tex1= new TLatex(-0.25,0.755,"Luminosity uncertainties");
            tex1->SetTextColor(1);
            tex1->SetTextSize(20);
            tex1->SetTextFont(43);
            tex1->Draw();
...
//            }
    ...
    if(SavePlot)c1->Print(Form("%s/%s%sIncJet%s%s%s%sInterpolatedRefEtaBin%dva.gif", plotsdir.Data(), val.Data(), algo.Data(),residual.Data(), ana.Data(), widebin.Data(), dep.Data(),netabin));
    if(SavePlot)c1->Print(Form("%s/%s%sIncJet%s%s%s%sInterpolatedRefEtaBin%dva.C", plotsdir.Data(), val.Data(), algo.Data(),residual.Data(), ana.Data(), widebin.Data(), dep.Data(),netabin));
    c1->Update();
...
//        else
//        TBox *b = new TBox(theX-5,val-err,theX+5,val+err);
